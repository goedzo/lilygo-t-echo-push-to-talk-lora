#line 1 "V:\\Bmad\\Project_ptt_lora\\main\\app_modes.cpp"
#include <AceButton.h>
#include <TinyGPS++.h>
#include "utilities.h"
...
void switchMode(String receivedMode) {
    Serial.print("[SWITCH] switchMode called from ");
    void* ret = __builtin_return_address(0);
    Serial.print("ret=0x"); Serial.println((uint32_t)ret, HEX);
    Serial.print("  old_mode="); Serial.print(current_mode);
    Serial.print(" new_mode="); Serial.println(receivedMode);

    // Guard: don't switch mode if no BLE connection and incoming mode matches what we'd get from button cycling
    // This prevents stray BLE writes (e.g. from old app connections or cached notifications) from overriding TST
    int newModeIndex = -1;
