...
- **App source:** `cordova_app/PTTLora/` — Cordova project root
  - `PTTLora/www/index.html` — entry page (mode selector with TXT pill first, chat-style conversation list below mode selector, alias moved to settings panel)
  - `PTTLora/www/js/index.js` — BLE + LoRa message logic. Sends `"SENDTXT:{message}"` for TXT mode, `switchMode()` sends mode name via BLE GATT. Device name filter: `/^LilygoT-Echo-[A-F0-9]{8}$/`. Mode pills in index.html map 1:1 to firmware modes. Named contacts: alias set via `SETNAME:{name}` GATT, buddy list sync via `GETBUDDY`/`SETBUDDY`, stored as CSV "CN{name}|DI{id},". Device settings panel: toggle via status bar gear button, fetches via `GETSETTINGS`, saves via `SETSETTINGS:KEY=val,...` (keyed format, partial updates supported). Display name / alias moved to settings panel (no longer on main screen). Chat UI uses per-conversation threading keyed by sender short ID, with unread badges and active chat view. Message storage is a `conversations` map (`{ shortId: { peerName, messages:[], unreadCount } }`) instead of flat message history. Active chat view renders threaded bubbles with send status icons. Toast notifications via `showToast()` for all user-facing feedback. Incoming messages trigger Android push notifications via `com.devdactic.cordovanotification` plugin when no thread is open. GETSCREEN uses staged retries (3 attempts with 300/600/1200ms backoff). PTT status polling via direct GATT write of `"GETSTATUS"` every 3s. Screen mirror sync updates TXT pill badge directly from `txt_lora_alive` field in screen payload.
...
- Device settings GATT actions:
  ...
  - `GETSCREEN` — triggers forced screen sync (bypasses all throttling). Device responds with `LINE:S|M:...` payload.
  - `GCCREN` — secondary screen sync trigger (alias for GETSCREEN compatibility). Triggers same forced sync path as GETSCREEN.
